...
import sys
import re
import cchardet as chardet # @UnresolvedImport
import codecs
import argparse
import logging
...
fileHandler = logging.FileHandler('/var/services/homes/admin/smi2srt/smi2srt.log')
...
def convertSMI(smi_file):
    if not os.path.exists(smi_file):
        ...
        return False
    rndx = smi_file.rfind('.')
    srt_file = '%s.ko.srt' % smi_file[0:rndx]
    ...
    try:
        encoding = chardet.detect(smi_sgml)
        smi_sgml = smi_sgml.decode(encoding['encoding'])
    except:
        logger.debug("[Failed] " + smi_file)
        return False
    ...
    ofp = codecs.open(srt_file, 'w', encoding='utf-8')
    ndx = 1
    for si in srt_list:
        si.convertSrt()
    ...
    for p, w, f in os.walk(PATH):
        for file_name in f:
            if file_name[-4:].lower() == '.smi':
                ...
                if convertSMI(os.path.join(p, file_name)):
                    print "Converting <%s> OK!" % file_name
                else:
                    print "Converting <%s> Failture!" % file_name
